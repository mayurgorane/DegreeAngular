...
export class DegreeServiceService {
  ...
  postDocument(formData:any):Observable<any>{
   return this.http.post<any>(`http://localhost:9090/api/documents/upload`,formData)
   
  }
  postNotes(notes:any[],degreeId:number):Observable<any>{
    return this.http.post<any>(`http://localhost:9090/degreeId/${degreeId}/notes`,notes);
    
   }

   getDocumentByDegreeId(degreeId: number): Observable<any> {
    return this.http.get<any>(`http://localhost:9090/api/documents/degree/${degreeId}`);
      
  }

  updateDegree(id: number, masterId: number, value: string, updatedDegree: any): Observable<any> {
    const url = `http://localhost:9090/degree/${id}/masterId/${masterId}/value/${value}`;
  ...
  updateDocument(degreeId: number,formData:any): Observable<any> {
    ...
    return this.http.put<any>(`http://localhost:9090/api/documents/${degreeId}/update`, formData)
      .pipe( 
        catchError(error => {
...
getNotes(degreeId: number): Observable<any[]> {
  return this.http.get<any[]>(`http://localhost:9090/degreeId/${degreeId}/notes/highestVersion`);
    
}

deletePostUpdateNotes(degreeId: number,data: any):Observable<any>{
  return this.http.post<any>(`http://localhost:9090/degreeId/${degreeId}/notes`,data);
